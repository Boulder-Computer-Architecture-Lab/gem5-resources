...
# for gcc version 10 or greater
sed -i 's/^#%define GCCge10/%define GCCge10/' /home/gem5/spec2017/config/myconfig.${ISA}.cfg
...
runcpu --config=myconfig.${ISA}.cfg --define build_ncpus=$(nproc) --define gcc_dir="/usr" --action=build all --tuning=base
runcpu --config=myconfig.${ISA}.cfg --define build_ncpus=$(nproc) --define gcc_dir="/usr" --action=setup all --tuning=base
